        ...
        <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
          <div style={{ position: 'relative' }}>
            ...
            <input
              ...
              style={{ padding: '0.5rem 0.75rem 0.5rem 2.2rem', borderRadius: '8px', backgroundColor: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', fontSize: '0.85rem' }}
            />
          </div>
        </div>
      </div>
            ...
            {filteredLeads.map((lead) => (
              <tr key={lead.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                <td style={{ padding: '0.85rem 1rem' }}>
                  <strong style={{ color: '#fff', display: 'block', fontSize: '0.92rem' }}>{lead.contactName || 'Anonymous Visitor'}</strong>
                  <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}>{lead.contactPhone} {lead.contactEmail ? `• ${lead.contactEmail}` : ''}</span>
                </td>
                <td style={{ padding: '0.85rem 1rem' }}>
                  <span style={{ padding: '0.2rem 0.5rem', borderRadius: '6px', fontSize: '0.75rem', fontWeight: 700, backgroundColor: 'rgba(59, 130, 246, 0.15)', color: '#60a5fa' }}>
                    {lead.channel}
                  </span>
                </td>
                <td style={{ padding: '0.85rem 1rem' }}>
                  <span style={{
                    ...
                    fontSize: '0.75rem',
                    fontWeight: 800,
                    backgroundColor: lead.status === 'CONVERTED' ? 'rgba(16, 185, 129, 0.2)' : lead.status === 'NEW' ? 'rgba(59, 130, 246, 0.2)' : 'rgba(245, 158, 11, 0.2)',
                    color: lead.status === 'CONVERTED' ? '#34d399' : lead.status === 'NEW' ? '#60a5fa' : '#fbbf24'
                  }}>
                    {lead.status}
                  </span>
                </td>
                <td style={{ padding: '0.85rem 1rem' }}>
                  <span style={{ fontSize: '0.78rem', fontWeight: 700, color: lead.priority === 'HIGH' ? '#f43f5e' : '#cbd5e1' }}>
                    {lead.priority === 'HIGH' ? '🔥 HIGH' : lead.priority}
                  </span>
                </td>
                <td style={{ padding: '0.85rem 1rem' }}>
                  {lead.saleAmount ? (
                    <strong style={{ color: '#34d399', fontSize: '0.95rem' }}>₹{lead.saleAmount.toLocaleString()}</strong>
                  ) : (
                ...
                <td style={{ padding: '0.85rem 1rem', textAlign: 'right' }}>
                  <div style={{ display: 'flex', gap: '0.4rem', justifyContent: 'flex-end' }}>
                    ...
                    <button
                      ...
                      title="View Details"
                      style={{ padding: '0.35rem 0.65rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.1)', backgroundColor: '#1e293b', color: '#fff', fontSize: '0.75rem', cursor: 'pointer' }}
                    >
                      View
                    </button>
                    <button
