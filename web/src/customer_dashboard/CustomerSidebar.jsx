...
function CustomerSidebar({
  ...
  const [openSubmenu, setOpenSubmenu] = useState(() => {
    return activeTab && activeTab.startsWith('explore') ? 'explore' : null;
  });
      ...
      <div style={{
          ...
          {isExpanded && (
            <div>
              <h1 style={{ fontSize: '1.15rem', fontWeight: 900, color: isDark ? '#fff' : '#0f172a', margin: 0, letterSpacing: '-0.02em', whiteSpace: 'nowrap' }}>
                ManaCity <span className="gradient-text">Member</span>
              </h1>
              <span style={{ fontSize: '0.68rem', color: isDark ? '#f472b6' : '#be185d', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 800 }}>
                Customer Portal
              </span>
            </div>
          )}
        ...
        {isExpanded && (
          <button
            ...
            style={{
              backgroundColor: isPinned ? (isDark ? 'rgba(236, 72, 153, 0.2)' : 'rgba(236, 72, 153, 0.15)') : 'transparent',
              border: 'none',
              borderRadius: '6px',
              padding: '0.35rem',
              cursor: 'pointer',
              color: isPinned ? (isDark ? '#f472b6' : '#be185d') : (isDark ? '#64748b' : '#94a3b8')
            }}
          >
            ...
            <div key={item.id} style={{ display: 'flex', flexDirection: 'column' }}>
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{
                    ...
                    borderRadius: '8px',
                    backgroundColor: isParentActive
                      ? (isDark ? 'rgba(236, 72, 153, 0.35)' : '#be185d')
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
                    {item.badge && (
                      <span style={{
                        ...
                        borderRadius: '10px',
                        backgroundColor: isParentActive ? (isDark ? '#ec4899' : '#be185d') : (isDark ? 'rgba(255, 255, 255, 0.08)' : '#e2e8f0'),
              ...
              {isExpanded && item.subItems && isSubOpen && (
                <div style={{
                  ...
                  borderLeft: isDark ? '2px solid rgba(236, 72, 153, 0.3)' : '2px solid #cbd5e1'
                }}>
                  {item.subItems.map(sub => {
                    ...
                    return (
                      ...
                      >
                        <SubIcon size={14} color={isSubActive ? (isDark ? '#f472b6' : '#be185d') : (isDark ? '#94a3b8' : '#64748b')} />
                        <span>{sub.label}</span>
                      </button>
                    );
                  })}
      ...
      {isExpanded && (
        <div style={{
          ...
          <span style={{ fontSize: '0.72rem', color: isDark ? '#94a3b8' : '#475569', display: 'flex', alignItems: 'center', gap: '0.35rem' }}>
            <Pin size={12} color={isPinned ? '#f472b6' : '#94a3b8'} />
